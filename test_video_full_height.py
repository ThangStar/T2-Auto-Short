"""
Test script for video export full height issue
"""
import sys
import os
...
def test_video_full_height():
    """Test video export full height issue"""
    print("Testing video export full height...")
    
    # Create test window
    root = tk.Tk()
    root.title("Video Full Height Test")
    ...
    def add_test_content():
        """Add test content for full height test"""
        # Clear existing layers
        timeline.layers.clear()
        
        # Add text layer at top
        text_layer = timeline.create_text_layer("TOP TEXT", 0.0, 5.0)
        ...
        text_layer2 = timeline.create_text_layer("BOTTOM TEXT", 0.0, 5.0)
        text_layer2.set_position(50, 1000)  # Near bottom of 1280 height
        text_layer2.set_size(200, 80)
        text_layer2.font_size = 24
        text_layer.font_color = "#FFFFFF"
        text_layer2.bg_color = "#00FF00"
        text_layer2.bg_opacity = 0.8
        
        # Add box layer in middle
        box_layer = timeline.create_box_layer(2.0, 7.0)
        box_layer.set_position(300, 500)  # Middle of 1280 height
        box_layer.set_size(150, 100)
        box_layer.fill_color = "#0000FF"
        box_layer.fill_opacity = 0.6
        box_layer.border_color = "#FFFFFF"
        box_layer.border_width = 2
        
        print("Added test content:")
        print(f"  - Top text at y=50")
        print(f"  - Bottom text at y=1000")
        print(f"  - Middle box at y=500")
    ...
    def export_test_video():
        """Export test video to check full height"""
        ...
        print(f"Output resolution: 720x1280 (9:16)")
    ...
    instructions = ttk.Label(
        main_frame,
        text="Instructions: 1) Add test content 2) Export video 3) Check if content fills full height (1280px)",
        font=("Arial", 10)
    )
    ...
    print("Video Full Height Test")
    print("=====================")
    print("✅ Test content at top, middle, and bottom")
    print("✅ Export resolution: 720x1280 (9:16)")
    print("✅ Check if content fills full height")
    print("✅ Debug FFmpeg command in console")
...
def main():
    """Main test function"""
    try:
        root = test_video_full_height()
        root.mainloop()
        return 0
    except Exception as e:
        print(f"Error in video full height test: {e}")
        import traceback
        traceback.print_exc()
        return 1
